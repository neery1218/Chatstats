...
public class ConversationThread {//holds array of conversations
    ...
    public ConversationThread(Cursor r, Cursor s, String address)//basic constructor
    {
        ...
        this.received = retrieve(r, SMS.Status.RECEIVED);
        this.sent = retrieve(s, SMS.Status.SENT);
    ...
    private void initializeResponses() {//sorts the SMS messages into blocks of responses
        int begin = 0, end = 0;
        SMS.Status key = messages.get(0).getStatus();
        Response temp;
    ...
    private ArrayList<SMS> retrieve(Cursor c, SMS.Status status) { //retrieves all messages from the cursor via query
